...
MODEL_FNAME = "tartanvo_{0:%Y%m%d_%H%M}.pkl"
...
def main(cfg):
    ...
    # Load configs
    yaml = YAML(typ='safe')
    with open("/drone/AttentionVO/configs/train.yaml", "r") as stream:
    ...
    train_loss = {k: 0 for k in ["flow", "pose", "total"]}
    ...
    for ep in trange(hparams["epoch"], desc="#Epoch", leave=False):
        ...
        for i, sample in enumerate(tqdm(train_loader, desc="#Train batch", leave=False)):
            ...
            batch_loss["total"] = total_criterion(
                batch_loss["flow"], batch_loss["pose"])
        ...
        with torch.no_grad():
            ...
            if metric[config["ckpnt_metric"]] < best_metirc:
                ...
                }, MODEL_DIR / cfg.log_name /MODEL_FNAME.format(datetime.now()))
