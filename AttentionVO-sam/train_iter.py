...
class Trainer():
    def __init__(self, cfg, args):
        ...
        self.train_loss = {k: 0 for k in ["flow","loss_R","loss_T","pose", "total"]}
    ...
    def Train_one_epoch(self):
        ...
        for i, sample in enumerate(tqdm(self.train_loader, desc="#Train batch", leave=False)):
            ...
            # Ground truth
            flow_gt = sample["flow"].to(self.device)  # N x C x H x W
            pose_gt = sample["motion"].to(self.device)  # N x 6

            # Forward
            flow, pose = self.model([img0, img1, intrinsic])

            # Batch loss
            # Pre-divide by batch size to make it numerically stable
            self.batch_loss["flow"] = self.flow_criterion(flow, flow_gt).sum()
            self.batch_loss["loss_T"], self.batch_loss["loss_R"] = self.pose_criterion(pose, pose_gt)
            self.batch_loss["pose"] = (self.batch_loss["loss_T"] + self.batch_loss["loss_R"])
            # self.batch_loss["pose"] = self.pose_criterion(pose, pose_gt).sum()
            self.batch_loss["total"] = self.total_criterion(
                self.batch_loss["flow"], self.batch_loss["pose"])
    ...
    def Validation(self):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(tqdm(self.valid_loader, desc="#Valid batch", leave=False)):
                ...
                self.batch_loss["loss_T"], self.batch_loss["loss_R"] = self.pose_criterion(pose, pose_gt)
                self.batch_loss["pose"] = (self.batch_loss["loss_T"] + self.batch_loss["loss_R"])
                self.batch_loss["total"] = self.total_criterion(self.batch_loss["flow"], self.batch_loss["pose"])
            ...
            if 1 and self.metric[self.config["ckpnt_metric"]] < self.best_metirc:
