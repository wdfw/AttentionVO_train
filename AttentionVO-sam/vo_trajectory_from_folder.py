...
if __name__ == '__main__':
    ...
    if args.save_flow:
        flowdir = '/drone/AttentionVO/results/'+testname+'_flow'
    ...
    while True:
        ...
        if args.save_flow:
            for k in range(flow.shape[0]):
                ...
                np.save(flowdir+'/'+str(flowcount).zfill(6)+'flow.npy',flowk)
                flow_vis = visflow(flowk)
                cv2.imwrite(flowdir+'/'+str(flowcount).zfill(6)+'flow.png',flow_vis)
                flowcount += 1
